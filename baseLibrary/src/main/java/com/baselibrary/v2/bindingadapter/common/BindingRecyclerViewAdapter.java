...
/**
 * A {@link } that binds items to layouts using the given {@link ItemView} or
...
public class BindingRecyclerViewAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements BindingCollectionAdapter<T> {
    ...
    public void onBindBinding(ViewDataBinding binding, int bindingVariable, @LayoutRes int layoutRes, int position, T item) {
        if (bindingVariable != ItemView.BINDING_VARIABLE_NONE) {
            ...
            if (item instanceof BViewModel){
                ((BViewModel<ViewDataBinding>)item).setBinding(binding);
            }
            if (!result) {
